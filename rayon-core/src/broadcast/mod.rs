    ...
    R: Send,
{
    ...
    // SAFETY: we only dereference the pointer if it is non-null (via `as_ref`).
    ...
    let jobs: Vec<_> = (0..n_threads)
        ...
        .collect();
    // SAFETY: each job lives on the stack until `into_result` below.
    ...
    // SAFETY: all jobs have completed (latch reached zero).
    jobs.into_iter()
        .map(|job| unsafe { job.into_result() })
        .collect()
}
